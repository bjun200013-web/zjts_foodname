...
白切鸡,A. 富含优质蛋白质的菜品,A2. 禽肉类,鸡肉,整鸡,鸡肉,煮,粤菜,宴席菜/大菜,无,记忆菜
油泼面,B. 以碳水化合物食材为主的菜品,B1. 精制谷物类,精面粉面食,面条,面粉或小麦类/葱蒜或洋葱类,其他,西北菜,小吃/街头美食,主食/淀粉类/高糖/高油,推理菜
扬州炒饭,B. 以碳水化合物食材为主的菜品,B1. 精制谷物类,精米饭/米粉/米制品,炒饭,米类/鸡蛋/虾类/猪肉,炒,苏菜,餐馆菜,主食/淀粉类/高糖/高油,记忆菜
汽车,不适用,不适用,不适用,不适用,不适用,不适用,不适用,不适用,不适用,不适用"""},
            ],
        }
    )
    logger.info("Context create complete")
...
if __name__ == "__main__":
    ...
    fd_target = Path(
        f"/date0/crwu/zjts_foodname/data/classification/outputs_by_type_{file_name}_{version}"
    )
