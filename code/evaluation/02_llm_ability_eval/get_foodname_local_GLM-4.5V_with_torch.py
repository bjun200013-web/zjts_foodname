    ...
    def log(self):
        ...
        for logical_idx, physical_id in enumerate(self.target_gpus):
            try:
                ...
                msg_to_print_list.append(f"GPU {physical_id}: 已分配 {mem_alloc:.2f}GB(delta={mem_alloc-self.last_mem_alloc_list[logical_idx]:.2f}), 已保留 {mem_resv:.2f}GB(delta={mem_resv-self.last_mem_resv_list[logical_idx]:.2f})")
...
def evaluate_model_accuracy(
    ...
    print(
        f"\nGenerating responses for {len(packed)} samples using model (torch): {model_path}"
    )
    # 分批推理，节省显存
    for i in tqdm(range(0, len(packed), batch_size), desc="Batches"):
        ...
        for meta, pred in zip(metas, preds):
            ...
            _a = _BOX_RE.search(pred)
            model_answer = _a.group(1).strip() if _a else ""
        ...
        # 可选的中途日志
        if total % 100 == 0:
            print(f"  Progress: {total}/{len(packed)} | Acc={correct/total*100:.2f}%")
...
def main():
    ap = argparse.ArgumentParser("Evaluate GLM-4.5V (Torch/HF) on Excel dataset.")
    ap.add_argument(
        "-p",
        "--model_path",
        type=str,
    ...
    ap.add_argument(
        "-o", "--output_dir", type=str, default="./evaluation_results", help="输出目录"
    )
    ap.add_argument(
        "--input_excel_path",
        type=str,
    ...
    try:
        ...
        print(
            f"[WARN] Glm4vMoeForConditionalGeneration failed, fallback to AutoModel: {e1}"
        )
    ...
    # # 一些 GLM 模型的权重实现放在 AutoModel（非 *ForCausalLM），因此做个兼容加载
    # model = None
    # try:
    #     model = Glm4vMoeForConditionalGeneration.from_pretrained(
    #         args.model_path,
    ...
    #     )
    # except Exception as e1:
    #     print(f"[WARN] Glm4vMoeForConditionalGeneration load failed, fallback to AutoModel: {e1}")
    ...
    # # 处理器与 tokenizer
    # processor = AutoProcessor.from_pretrained(args.model_path, trust_remote_code=True)
    # # 有些 processor 带 tokenizer，有些需要单独取
