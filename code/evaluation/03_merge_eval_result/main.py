...
def evaluate_wrapper(args):
    ...
    包装函数，用于进程池调用
    """
    index, row_num, model_name, real, predicted, output_path = args
    # 在每个子进程中重新创建客户端
    client = OpenAI(
        base_url=API_URL,
    ...
    try:
        ...
        if score is None:
            ...
            return (index, row_num, real, predicted, model_name, None, False)
        data = (row_num, real, predicted, model_name, score)        
        logger.info(f'菜品: {real}，模型预测: {predicted}，模型{model_name}打分: {score}')
        pk_dump(data, index, output_path)
        return (index, row_num, real, predicted, model_name, score, True)
...
if __name__ == "__main__":
    ...
    columns = (
        ["ground_truth_answer", "model_prediction"]
        + [model + "的评分" for model in models]
        + [
            "first_level",
            "second_level",
            "main_ingredients",
            "cuisine_region_classification",
            "cooking_method",
            "consumption_scene",
            "nutritional_advice_categories",
        ]
    )
    ...
    # 按model顺序评估，那么0~num-1是models[0]的评分，第x菜的第y个model评分就是start*num*len(models) + (y-1)*num + (x-1)        
    tasks = []
    target_index_list = list(
        range(start * num * len(models), start * num * len(models) + len(real_dishes) * len(models))
    )
    ...
    for index in target_index_list:                
        model = models[(index - start * num * len(models)) // len(real_dishes)]
        i = (index - start * num * len(models)) % len(real_dishes)
        real = real_dishes.values[i]
        pred = predicted_dishes.values[i]
        
        if pd.isna(real) or pd.isna(pred):
            # 直接处理空值情况
            pk_dump((i, real, pred, model, 0), index, pk_path)
            pbar.update(1)
            continue
        if exact_match(real, pred):
            # 直接处理完全匹配情况
            pk_dump((i, real, pred, model, 5), index, pk_path)
            pbar.update(1)
            continue
        
        # 需要API调用的任务
        tasks.append((index, i, model, real, pred, pk_path))
        ...
        for i in range(0, len(tasks), batch_size):
            ...
            with Pool(min(batch_size, len(batch_tasks))) as pool:
                ...
                for result in pool.imap_unordered(evaluate_wrapper, batch_tasks):
                    index, row_num, real, pred, model, score, success = result
                    if success and score is not None:
                        # 结果已经在 evaluate_wrapper 中保存到文件了
                        pass
                    pbar.update(1)
                    completed_count += 1
                    pbar.set_description(f"API调用进度 ({completed_count}/{len(target_index_list)})")
        # 检查已完成的pk并重试
        target_index_list = list(set(target_index_list) - set(search_pk(pk_path)))
        logger.info(f'剩余未完成的菜品: {target_index_list}')
    ...
    for pk_index in pk_indexs:
        i, real, pred, model, res = pk_load(pk_index, pk_path)
        results_df.loc[i, model + "的评分"] = res
